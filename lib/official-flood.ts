...
const COAST_URL =
  'http://wri-projects.s3.amazonaws.com/AqueductFloodTool/download/v2/inuncoast_historical_wtsub_hist_rp0100_0.tif';
const CACHE_TTL_MS = 30 * 24 * 3600 * 1000;
const GRID_N = 48;
const DEPTH_MAX_CM = 300;

const NOTE =
  'Официальная глобальная модель: WRI Aqueduct (GLOFRIS) — глубина паводка «раз в 100 лет», max(речной, прибрежный с учётом проседания грунта). Разрешение ~1 км, БЕЗ локальной инженерной защиты. Ливни/дренаж не входят — см. маски «Риск разлива рек» и «Ливневое подтопление».';
...
export async function computeOfficialFloodMask(ctx: MaskContext): Promise<MaskField> {
  ...
  const key =
    'q100:' + createHash('sha1').update(`${lat.toFixed(5)},${lon.toFixed(5)},${radius},v2`).digest('hex');
  const cached = await cacheGet<MaskField>(key);
  if (cached != null) {
    console.log('[q100] кэш ✓ WRI Aqueduct');
    return cached;
  }
  ...
  console.log(`[q100] WRI Aqueduct RP100 (река+море) · bbox ±${radius} м · ${lat.toFixed(4)}, ${lon.toFixed(4)}`);
  const [river, coast] = await Promise.all([
    sampleRaster(RIVER_URL, cells).catch(() => cells.map(() => null)),
    sampleRaster(COAST_URL, cells).catch(() => cells.map(() => null)),
  ]);
