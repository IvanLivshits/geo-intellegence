...
function isoCity(
  ...
  let half: number;
  if (meta.zone) {
    half = Math.min(payload.radius, Math.max(...meta.zone.map(([lo, la]) => {
  ...
  const toScreen = (p: [number, number], z: number): [number, number] => {
    const [px, py] = proj(p, z);
    return [
      cx + (px - (minX + maxX) / 2) * scale,
      cy + 10 + (py - (minY + maxY) / 2) * scale,
    ];
  };
  ...
  if (meta.zone) {
    const zonePts = meta.zone.map(([lo, la]) =>
      toScreen(localMetres(clat, clon, la, lo), 0),
    );
    parts.push(
...
function buildArt(meta: ShareMeta | null, payload: ScanPayload | null, seedHex: string): string {
  ...
  return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    <defs>
      ...
      <filter id="heavy" x="-80%" y="-80%" width="260%" height="260%">
